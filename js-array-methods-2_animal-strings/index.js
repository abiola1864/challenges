...
const catStartingFromIndexFiveExists = animalStrings.findIndex(
  (animal, index) => index === 0 && animal === "cat"
);
...
//   .map(animal => animal.length) // Creates an array with the length values of all strings
//   .reduce((total, length) => total + length, 0); // Sums up the length values using reduce()

// console.log(sumOfAllAnimalCharacters1); // Output: The sum of all the characters' lengths
